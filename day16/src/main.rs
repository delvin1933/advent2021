...
fn parse_packet(input: &str) -> (Packet, &str) {
    ...
    match PacketType::from_str(&input[3..6]) {
        PacketType::Literal => {
            ...
            println!("LITERAL : {}", u64::from_str_radix(&binary_rep, 2).unwrap());
...
fn compute_packet(packet: &Packet) -> u64 {
    match packet.packet_type {
        PacketType::Literal => packet.literal_value.unwrap(),
        PacketType::Operator(0) => {
            let sub_packets: &Vec<Packet> = &packet.operations_packets.as_ref().unwrap();
            sub_packets
                .iter()
                .fold(0, |acc, el| acc + compute_packet(el))
        } //Sum
        ...
        PacketType::Operator(2) => {
            ...
            sub_packets.fold(u64::MAX, |r, p| {
                if r < compute_packet(p) {
                    r
                } else {
                    compute_packet(p)
                }
...
fn main() {
    ...
    println!("PART2 : {}", u64::MAX - compute_packet(&pack));
}
